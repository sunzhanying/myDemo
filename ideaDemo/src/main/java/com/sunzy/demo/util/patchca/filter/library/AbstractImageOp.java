 ...
 */
package com.sunzy.demo.util.patchca.filter.library;
...
public abstract class AbstractImageOp implements BufferedImageOp {
	...
	protected int getPixel(int pixels[], int x, int y, int width, int height, int edgeMode) {
		if (x >= 0 && x < width && y >= 0 && y < height) {
			...
			if (x < 0) {
				...
				x = width + width - x - 1;//NOSONAR
			}
			if (y < 0) {
				y = -y;//NOSONAR
			} else if (y > height) {
				y = height = height - y - 1;//NOSONAR
			}
			try {
				return pixels[x + y * width];
			} catch (Exception e) {
	...
	public BufferedImage filter(BufferedImage src, BufferedImage dest) {
		if (dest == null) {
			dest = createCompatibleDestImage(src, null);//NOSONAR
		}
	...
	public BufferedImage createCompatibleDestImage(BufferedImage src, ColorModel destCM) {
		if (destCM == null) {
			destCM = src.getColorModel();//NOSONAR
			if (destCM instanceof IndexColorModel) {
				destCM = ColorModel.getRGBdefault();//NOSONAR
			}
		}
	...
	public Point2D getPoint2D(Point2D srcPt, Point2D dstPt) {
		if (dstPt == null) {
			dstPt = new Point2D.Float();//NOSONAR
		}
